...
function Player:init(map)
    ...
    for y = self.y + self.height, self.map.mapHeight do
        self.map:setTile(self.x, y, TILE_BRICK)
    end
...
function Player:update(dt)
    ...
    if self.dy < 0 then
        if self.map:tileAt(self.x, self.y).id ~= TILE_EMPTY or
            ...
            -- if self.map:tileAt(self.x, self.y).id == FLAG_UP or self.map:tileAt(self.x, self.y).id == POLE_TOP or self.map:tileAt(self.x, self.y).id == POLE_MID or self.map:tileAt(self.x, self.y).id == POLE_BOT  then
            --     gameState = 'completed'
            -- end
     ...
     -- check if player collided with victory flag 
    if self.map:tileAt(self.x, self.y).id == FLAG_UP or self.map:tileAt(self.x, self.y).id == POLE_TOP or self.map:tileAt(self.x - 1, self.y + 1).id == POLE_MID or self.map:tileAt(self.x + self.width + 1, self.y ).id == POLE_MID or self.map:tileAt(self.x, self.y).id == POLE_BOT  then
        -- self.map:setTile(self.x, self.y - 1, FLAG_INVERT)
        gameState = 'completed'
